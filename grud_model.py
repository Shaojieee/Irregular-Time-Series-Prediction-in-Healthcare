...
from data import load_mtand_mortality_dataset as load_strats_mtand_mortality_dataset
...
def generate_grud_mortality(data_dir, output_dir):
    X,Y = {},{}
    X['train'], Y['train'], X['val'], Y['val'], X['test'], Y['test'], D, V, max_len, len_time_key = load_strats_mtand_mortality_dataset(data_dir, max_time=2, len_time_query=2)
    ...
    for mode in ['train', 'val', 'test']:
        ...
        times = X[mode][4]
        values = X[mode][5]
        measurements = X[mode][6].astype(np.bool)
        label = Y[mode]
        lengths = np.sum(np.any(measurements ,axis=-1),axis=-1)

        assert lengths.shape==label.shape
...
def load_grud_mortality_dataset(data_path):
...
if __name__=='__main__':
    ...
    generate_grud_mortality(
        data_dir='./data_physionet/strats_mtand', 
        output_dir='./data_physionet/grud',
    )

    generate_grud_mortality(
        data_dir='./data_mimic/strats_mtand', 
        output_dir='./data_mimic/grud',
    )