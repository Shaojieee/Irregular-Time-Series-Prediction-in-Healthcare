...
from data import load_mortality_dataset, load_mtand_mortality_dataset
...
def train(args):
    ...
    if 'mtand' in args.model_type:
        train_ip, train_op, valid_ip, valid_op, test_ip, test_op, D, V, max_len, len_time_key = load_mtand_mortality_dataset(args.data_dir, max_time=max_time, with_demo=args.with_demo, len_time_query=len_time_query)
    else:
        train_ip, train_op, valid_ip, valid_op, test_ip, test_op, D, V, max_len = load_mortality_dataset(args.data_dir, with_demo=args.with_demo, with_imputation=args.with_imputation)
    ...
    for ld in lds:
        ...
        for i in range(repeats[ld]):
            ...
            if args.model_type=='mtand_strats':
