...
from grud_model import load_grud_mortality_dataset
from seft_utils import (
...
def generate_seft_mortality_data(data_path, output_dir):
    ...
    ip = {}
    op = {}
    ip['train'], op['train'], ip['val'], op['val'], ip['test'], op['test'] = load_grud_mortality_dataset(data_path)
    ...
    max_len = int(np.quantile(ts_len, 1.0))
        ...
        for i in range(len(processed_ip[mode])):
            if i not in [0, 4]:
                for j in range(len(op[mode])):
                    if i==3:
                        if max_len-processed_ip[mode][i][j].shape[0]>0:
                            processed_ip[mode][i][j] = np.pad(processed_ip[mode][i][j], (0,max_len-processed_ip[mode][i][j].shape[0]))
                        elif max_len-processed_ip[mode][i][j].shape[0]<0:
                            processed_ip[mode][i][j] = processed_ip[mode][i][j][:max_len]
                    else:
                        if max_len-processed_ip[mode][i][j].shape[0]>0:
    ...
    for mode in ['train', 'val', 'test']:
        for i in range(len(processed_ip[mode])):
            print(processed_ip[mode][i].shape)
...
def load_seft_mortality_dataset(data_path):
...
if __name__=='__main__':
    ...
    generate_seft_mortality_data(
        data_path='./data_physionet/grud', 
        output_dir='./data_physionet/seft'
    )

    generate_seft_mortality_data(
        data_path='./data_mimic/grud', 
        output_dir='./data_mimic/seft'
    )